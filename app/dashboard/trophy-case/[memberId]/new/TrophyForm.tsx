...
export function TrophyForm({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = trophyId
        ? await updateTrophy(trophyId, fd)
        : await createTrophy(fd);
      ...
      if (onDone) {
        ...
        router.push(`/dashboard/trophy-case/${defaultMemberId}/${result.id}`);
      }
      router.refresh();
    });
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] px-4 py-6 sm:px-6">
      ...
      {/* Title */}
      <div>
        <label htmlFor="trophy-title" className="mb-1 block text-sm font-medium text-[var(--foreground)]">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="trophy-by" className="mb-1 block text-sm font-medium text-[var(--foreground)]">
            Awarded by{" "}
        ...
        <div>
          <label htmlFor="trophy-date" className="mb-1 block text-sm font-medium text-[var(--foreground)]">
            Date received{" "}
            <span className="font-normal text-xs text-[var(--muted)]">(optional)</span>
          </label>
          <input
      ...
      {/* Description */}
      <div>
        <label htmlFor="trophy-desc" className="mb-1 block text-sm font-medium text-[var(--foreground)]">
          Description{" "}
          <span className="font-normal text-xs text-[var(--muted)]">(optional)</span>
        </label>
        <textarea
          ...
          placeholder="The story behind the trophy, what it means, any details worth remembering…"
      ...
      {allMembers.length > 1 && (
        <div>
          ...
          <p className="mt-1.5 text-xs text-[var(--muted)]">
            Select everyone who received this trophy.
          </p>
        </div>
      )}
