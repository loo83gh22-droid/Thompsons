...
import { addStoryPerspective, removeStoryPerspective } from "./actions";
...
export function StoryPerspectives({ storyId }: { storyId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      await addStoryPerspective(storyId, content.trim(), memberId);
      const supabase = createClient();
      const { data } = await supabase
        .from("story_perspectives")
        .select("id, content, family_member_id, family_members!family_member_id(name)")
    ...
    toast("Remove this perspective?", {
      action: {
        ...
        onClick: async () => {
          setDeletingId(id);
          try {
            await removeStoryPerspective(id);
  ...
  return (
    <div className="mt-12 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6">
      ...
      <p className="mt-1 text-sm text-[var(--muted)]">
        Different angles on the same memory — add how others experienced this story.
      </p>
