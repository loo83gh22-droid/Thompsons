...
async function getNextMosaicSortOrder(supabase: SupabaseClient) {
...
export async function createJournalEntry(formData: FormData) {
  ...
  // If location provided, geocode and create map pin
  if (location?.trim()) {
    try {
  ...
  let mosaicOrder = await getNextMosaicSortOrder(supabase);
