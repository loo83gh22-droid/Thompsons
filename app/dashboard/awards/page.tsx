...
export default async function AwardsPage() {
  ...
  // All awards for this family (sorted newest first for cover images)
  ...
  // All award_members for this family's awards
  ...
  // Build count and cover-image maps per member
  ...
  // allAwards is sorted newest-first; first encounter per member = most recent cover
  for (const award of allAwards ?? []) {
  ...
  const membersWithAwards = (members ?? []).filter((m) => (countByMember[m.id] ?? 0) > 0);
  const membersWithoutAwards = (members ?? []).filter((m) => (countByMember[m.id] ?? 0) === 0);
  ...
  return (
    <div>
      ...
      {membersWithAwards.length === 0 && membersWithoutAwards.length === 0 ? (
        <EmptyState
          icon="🏆"
          headline="No family members yet"
          description="Add family members first, then start recording their awards and achievements."
          actionLabel="Add members"
          actionHref="/dashboard/our-family"
        />
      ) : membersWithAwards.length === 0 ? (
        <div className="space-y-8">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {membersWithoutAwards.map((member) => (
                <Link
                  key={member.id}
                  href={`/dashboard/awards/${member.id}/new`}
                ...
                >
                  <div className="relative aspect-[4/3] w-full bg-[var(--surface)]">
                    {cover ? (
                      <Image
                        src={cover}
                        alt={`${member.nickname || member.name}'s awards`}
                        fill
                        unoptimized
